...
vector1[3] # what is the third element of vecotr 1

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # default is by column
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # changes default to row
matrix2
...
```{r}
durham_high_temp_f <- c(48,52,52,50,51,51,50,45,39,37)
durham_low_temp_f <- c(24,38,27,31,28,35,36,30,21,20)
```
...
durham_high_temp_c <- conversion_f_to_c(durham_high_temp_f)
durham_high_temp_c
durham_low_temp_c <- conversion_f_to_c(durham_low_temp_f)
durham_low_temp_c
```
...
```{r}
durham_forecast <- data.frame(durham_high_temp_f,durham_low_temp_f, durham_high_temp_c, durham_low_temp_c)
names(durham_forecast) <- c("High (F)", "Low (F)", "High (F)", "Low (C)")
print(durham_forecast)
```
